...
class SAVView(discord.ui.View):
    ...
    async def create_ticket(self, interaction, type_t):
        ...
        await channel.send(f"Bonjour {interaction.user.mention}, un responsable va s'occuper de toi ici.")
...
class CoopView(discord.ui.View):
    ...
    async def create_coop_thread(self, interaction, label):
        ...
        await thread.send(f"{interaction.user.mention} a besoin d'aide pour : **{label}**. Qui est dispo ?", view=view)
        await interaction.response.send_message("Demande créée !", ephemeral=True)
